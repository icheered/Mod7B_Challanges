...
#include "DummyRoutingProtocol.h"

namespace my_protocol {

    DummyRoutingProtocol::DummyRoutingProtocol() {
    }

    DummyRoutingProtocol::~DummyRoutingProtocol() {
    }

    void DummyRoutingProtocol::init(framework::LinkLayer *linkLayer) {
        this->linkLayer = linkLayer;
    }

    void DummyRoutingProtocol::tick(std::vector<framework::PacketWithLinkCost> packetsWithLinkCosts) {
    ...
    std::map<int32_t, int32_t> DummyRoutingProtocol::getForwardingTable() {
        ...
        for (std::map<int32_t, DummyRoute>::iterator it = myRoutingTable.begin(); it != myRoutingTable.end(); it++) {
            int32_t destination = it->first;

            DummyRoute r = it->second;
