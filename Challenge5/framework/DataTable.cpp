...
#include "DataTable.h"
namespace framework {
    DataTable::DataTable(uint32_t nColumns) {
        if (nColumns < 0) {
    ...
    uint32_t DataTable::getNColumns() {
        return this->columns;
    }

    uint32_t DataTable::getNRows() {
        return this->data.size();
    }

    void DataTable::set(uint32_t row, uint32_t column, int32_t value) {
    ...
    int32_t DataTable::get(uint32_t row, uint32_t column) {
        if (column < 0 || column >= this->columns) {
    ...
    void DataTable::setRow(uint32_t row, std::vector<int32_t> values) {
    ...
    std::vector<int32_t> DataTable::getRow(uint32_t row) {
        if (row < 0 || row >= this->data.size()) {
            std::cerr << "Row index (" << std::to_string(row) << ") out of range (0.." << std::to_string(this->data.size()) << ")." << std::endl;
            exit(1);
        }
